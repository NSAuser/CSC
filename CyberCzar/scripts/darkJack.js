 ...
 function dealHand() {
 	...
 	if (isNaN(n)) {
 		...
 		alert("You don't have the LTC to bet that much.");
 	}
 	else {
 	...
 	if (n > 21) {
 		...
 		if (dealer21) {
 			...
 			if (player21) {
 				document.getElementById("standBtn").style.display = "none";
 				document.getElementById("hitBtn").style.display = "none";
 				msg = "Looks like we both got blackjack. That's a tie.";
 			}
 			else {
 				document.getElementById("standBtn").style.display = "none";
 				document.getElementById("hitBtn").style.display = "none";
 				msg = "I have blackjack, you lose.";
 				LTCoin -= bet;
 			}
 		}
 		else if (player21) {
 			document.getElementById("standBtn").style.display = "none";
 			document.getElementById("hitBtn").style.display = "none";
 			msg = "You got blackjack? You win " + formatLTC(bet * 3 / 2) + " LTC...";
 			LTCoin += bet * 3 / 2;
 		}
 	}
 	updateDealerInfo();
 ...
 function hit(who) {
 	...
 	if ((sum > 21) && (who == PLAYER)) {
 		document.getElementById("standBtn").style.display = "none";
 		document.getElementById("hitBtn").style.display = "none";
 		handFinished("Your total went over 21, you lose.");
 		LTCoin -= bet;
 		updateDealerInfo();
 	}
 }
 ...
 function dealerPlays() {
 	...
 	if (sum > 21) {
 		handFinished("My total went above 21. You win " + formatLTC(bet) + " LTC...");
 		LTCoin += bet;
 	}
 		...
 		handFinished("Dealer wins this one.");
 		LTCoin -= bet;
 	}
 	updateDealerInfo();
 }
